...
class UserHandler {
	...
	async fetch(filter = { _id: this.id }, opts = { multiple: false, setter: true }) {
		...
		const query = await users
			...
			.sort((a, b) => a._id.getTimestamp() - b._id.getTimestamp()); //if there are multiple entries, they are sorted oldest entry to newest

		//big ol ternary chain
		const result =
			query.length === 1 //check if query length is 1
				? query[0] //if true, return result
				: query.length > 1 //if false, check if query length is greater than 1
				...
				: null; //if query length is zero, return null

		//return result, if "setter" option is set, then set this.document to the result
	 ...
	 * @param {{username: string, password: string, ...anythingElse}} userDoc User's object, must have "username", and "password" properties
	 */
	async create(userDoc) {
		...
		existingUser !== null
			? (this.alreadyExists = true) //if existing user exists, set alreadyExists to true
	...
	async delete(filter = { _id: this.id }) {
		...
		//set document to undefined, unless this weird check fails. I can't remember why that check is there but I'm too afraid to remove it.
